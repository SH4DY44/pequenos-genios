...
function NotificationDemo() {
  const { crearNotificacion, cargarNotificaciones } = useNotifications();
  ...
  const crearNotificacionPrueba = async (tipo) => {
    ...
    try {
      ...
      await crearNotificacion(datosDemo[tipo]);
  ...
  const probarWhatsApp = () => {
    ...
    if (telefono) {
      WhatsAppService.notificarRapido(
        telefono,
        'Demo',
        'Esta es una prueba del sistema de notificaciones de Pequeños Genios!'
      );
